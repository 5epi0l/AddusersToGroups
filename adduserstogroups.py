...
parser.add_argument('--dc-ip', help='IP or FQDN of the Domain Controller (required for Kerberos).')
...
if not ad_password and not ad_hash and not use_kerberos:
    ...
    sys.exit(1)
if use_kerberos and not args.dc_ip:
    print('[-] Error: --dc-ip is required when using Kerberos authentication.')
    sys.exit(1)
# Construct search base
search_base = 'dc=' + ',dc='.join(domain_name.split('.'))
# Prepare connection
server_host = args.dc_ip if use_kerberos else domain_name
